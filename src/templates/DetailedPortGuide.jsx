...
const PORT_SECTIONS = [
  ...
  { key: 'boatTrips', label: 'Boat Trips', icon: Anchor },
...
export function DetailedPortGuide({ slug, portName, detailedContent, port }) {
  ...
  // Get boat trips from port object
  const boatTrips = port?.boatTrips || [];
  ...
  const hasContent = {
    ...
    boatTrips: boatTrips && boatTrips.length > 0,
  ...
  const renderSectionContent = () => {
    ...
    switch (activeSection) {
      ...
      case 'goFurther':
        ...
        break;
      case 'boatTrips':
        content = <BoatTripsSection boatTrips={boatTrips} portName={portName} />;
        break;
      case 'withKids':
...
function BoatTripsSection({ boatTrips, portName }) {
  if (!boatTrips || boatTrips.length === 0) {
    return <p>No boat trip information available yet.</p>;
  }

  return (
    <div className="section-boat-trips">
      <div className="section-intro">
        <h2>Boat Trips & Sea Excursions</h2>
        <p>Explore {portName} from the water with these boat trip options</p>
      </div>

      <hr className="section-divider" />

      {boatTrips.map((trip, idx) => (
        <Fragment key={idx}>
          <div className="boat-trip-card">
            <div className="boat-trip-header">
              <h3>{trip.title}</h3>
              {trip.duration && <span className="duration-badge">{trip.duration}</span>}
            </div>
            
            {trip.description && (
              <p className="boat-trip-description">{trip.description}</p>
            )}
            
            {trip.departFrom && (
              <div className="boat-trip-detail">
                <strong>Departure:</strong> {trip.departFrom}
              </div>
            )}
            
            {trip.highlights && trip.highlights.length > 0 && (
              <div className="boat-trip-highlights">
            ...
            {trip.bookingAdvice && (
              <div className="boat-trip-booking">
                <strong>Booking:</strong> {trip.bookingAdvice}
              </div>
            )}
          </div>
          {idx < boatTrips.length - 1 && <hr className="section-divider" />}
        </Fragment>
      ))}
    </div>
  );
